            ...
            @foreach ($events as $event)
                <div class="row  w-100">
                    <div class="col d-flex justify-content-between align-items-center">
                        <img src="{{ asset('storage/' . json_decode($event->event_image)[0]) }}" alt="profile"
                            width="40">
                        <h5>{{ $event->nama }}</h5>
                        <div class="status">
                        ...
                        <div class="button-action d-flex gap-4">
                            ...
                            @if (auth()->user()->canany(['admin-approve', 'admin-reject']))
                                <form action="{{ route('admin.event.approve', $event->id_event) }}" method="POST">
                                    ...
                                    @method('PUT')
                                    <button type="submit"
                                        class="rounded-3 px-4 py-2 bg-green-400 h-full">Izinkan</button>
                                </form>
                                <form action="{{ route('admin.event.reject', $event->id_event) }}" method="POST"
                                    ...
                                    @method('PUT')
                                    <button type="submit"
                                        class="rounded-3 px-4 py-2 bg-red-400 h-full">Hentikan</button>
                                </form>
                            @endif
                        </div>
                    </div>
